...
class Hybrid_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		$this->content = gridlove_fallback_gallery_filter( $this->content );
	...
	public function do_block_media() {
		...
		if ( !empty( $blocks ) ) {
			...
			foreach ( $blocks as $block ) {
				...
				if ( $block['blockName'] == 'core/'.$this->type || ( isset($block['attrs']['type']) && in_array( $block['attrs']['type'], array($this->type, 'rich' ) ) ) ) {

					$this->media = apply_filters('the_content', render_block( $block ) );
					$this->original_media = $block['innerHTML'];
					break;
				}
			}
		}
