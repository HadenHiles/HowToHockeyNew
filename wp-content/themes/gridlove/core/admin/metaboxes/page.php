		   		...
		   		<ul class="sortable">
					<?php $cats = gridlove_sort_option_items( $cats,  $module['cat']); ?>
			   		<?php foreach ( $cats as $cat ) : ?>
			   			...
			   			<li>
			   				<label>
			   					<input class="gridlove-count-me" type="checkbox" name="<?php echo esc_attr($name_prefix); ?>[cat][]" value="<?php echo esc_attr($cat->term_id); ?>" <?php echo esc_attr($checked); ?> /><?php echo esc_html( $cat->name );?>
			   				</label>
			   			</li>
			   		<?php endforeach; ?>
			   	</ul>
	...
	<div class="gridlove-opt-box gridlove-show-hide <?php echo esc_attr($show_hide_class); ?>">
		...
		<?php foreach ( $post_types as $post_type ) :
			...
			foreach ( $post_type->taxonomies as $taxonomy ) :
                ...
                <div class="gridlove-opt-inline gridlove-watch-for-changes gridlove-opt-taxonomies" data-watch="gridlove-fa-post-type" data-show-on-value="<?php echo esc_attr($post_type->name);?>">
                    <div class="gridlove-opt-title">
						...
						if ( $taxonomy['hierarchical'] ):
                            ...
                            <div class="gridlove-fit-height">
								<?php foreach ( $taxonomy['terms'] as $term ) : ?>
                                    ...
                                    <label><input class="gridlove-count-me" type="checkbox" name="<?php echo esc_attr( $name_prefix . '[' . $taxonomy_id . ']' ); ?>[]" value="<?php echo esc_attr( $term->term_id ); ?>" <?php echo esc_attr( $checked ); ?> /><?php echo esc_html( $term->name ); ?>
                                    </label>
                                    <br/>
								<?php endforeach; ?>
                            </div><br>
		...
		<div class="gridlove-opt-inline">
			...
			<div class="gridlove-opt-content">
				...
				<div class="gridlove-content-row">
					...
					<a href="#" class="gridlove-select-bg-image button"><?php esc_html_e('Upload', 'gridlove'); ?></a>
				</div>
			</div>
		</div>
	</div>
...
if ( !function_exists( 'gridlove_author_options_metabox' ) ) :
	function gridlove_author_options_metabox( $object, $box ) {
		...
		$orderby_options = array(
			'post_count' => esc_html__('Post Count', 'gridlove'),
			'user_name' => esc_html__('Username', 'gridlove'),
			'display_name' => esc_html__('Display Name', 'gridlove'),
			'user_registered' => esc_html__('Register Date', 'gridlove'),
		);
		
		?>
		<p><strong><?php esc_html_e('Order by', 'gridlove');?></strong></p>
		<?php 
		foreach ($orderby_options as $value => $name): ?>
			<?php $checked = ($authors_meta['orderby'] === $value) ? 'checked' : '' ; ?>
			<input type="radio" name="gridlove[authors][orderby]" value="<?php echo esc_attr($value); ?>" <?php echo esc_attr( $checked ); ?>>
			<label for="gridlove[authors][orderby]"><?php echo esc_html( $name ); ?></label><br>
		<?php endforeach; ?>
		...
		foreach($roles as $role) : ?>
			...
			<label for="gridlove[authors][roles]"><?php echo esc_html( $role ); ?></label><br>
		<?php endforeach; ?>
		...
		<div class="gridlove-opt">
	     ...
	     <div class="gridlove-opt-inline">
	   	...
	   	<?php foreach ( $taxonomies as $taxonomy ) : ?>
		    <div class="gridlove-opt">
				<div class="gridlove-opt-title">
					<?php esc_html_e( 'In ', 'gridlove' ); ?><?php echo esc_html( $taxonomy['name'] ); ?>:
				</div>
				<div class="gridlove-opt-content">
					...
					<?php if($taxonomy['hierarchical']) : ?>
						...
						<div class="gridlove-fit-height">
				   			...
				   			<label><input class="gridlove-count-me" type="checkbox" name="<?php echo esc_attr($name_prefix); ?>[tax][<?php echo esc_attr($taxonomy['id']); ?>][]" value="<?php echo esc_attr($term->term_id); ?>" <?php echo esc_attr( $checked ); ?> /><?php echo esc_html( $term->name );?></label><br/>
					   		<?php endforeach; ?>
				   		</div>
			   			<small class="howto"><?php esc_html_e( 'Check whether you want to display cpt from specific', 'gridlove' ); ?> <?php echo esc_html( $taxonomy['name'] ); ?></small>
